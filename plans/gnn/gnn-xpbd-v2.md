...
### 0.3 目标重构：评价体系换成痛点导向（第三轮）

前两轮修订都在优化**同一个指标**：对 HOOD teacher 的距离。这一轮改的是那个指标本身。

本项目不选 ChaosCloth 的理由从来不是"HOOD 更准"，而是三条工程痛点：

1. **配置繁琐** —— 逐资产手工调 stiffness / max distance / backstop / tether，或者用骨骼链
   方案，都是大量人工成本；
2. **抗不住高速运动与复杂动作**；
3. **穿模之后回不到好的 Pose**。

**teacher-relative score 对这三条一条都不构成代理。** 它衡量"能否廉价复现一个特定的 44 ms
求解器"，那是一个更窄的命题；一个纯 XPBD 求解器给出物理上合理但与 HOOD 不同的垂坠，这把尺子
会因为"不同"而罚它。所以：

| 处置 | 条目 |
| --- | --- |
| **降级为回归检查** | §8.1 的 teacher-relative score。它便宜、抓到过真问题（噪声底、模型对照），保留；但不再是主指标 |
| **升为主指标** | §8.0（新增）不引用 teacher 的三组指标：恢复力、速度断裂点、逐资产参数计数 |
| **升为核心论证** | §5.1 的 **Jᵀ 零空间**结论。它原本是"warm start 无用"的负面结果，实际上是**痛点 3 的机制证明** |
| **作废** | "等预算纯 XPBD 若追平则砍掉 GNN"这条判据。它只在 `over` 这一个维度上比较，而 `over` 恰是 XPBD 唯一看得见的维度 |
| **改为可替换性问题** | 正确的版本不是"GNN 是否必要"，而是"**某个** ML 前驱是否必要、我们这个是否可换"（**S13**，MNN） |

#### ❌ 已被 S10 推翻：零空间**不是**痛点 3 的机制

> **2026-08-20 撤回。** 下面这段论证在写入后当天被 `results/RECOVERY_SPEED_RESULTS.md` §1
> 的实测推翻。原文保留（划掉），因为错在哪本身是有价值的。

~~§5.1 已经证明并用单元测试固定住：**Jᵀ 的零空间包含所有不改变边长的形变**。而"缠绕、折叠、
翻到身体里侧"恰恰全是等距的 —— 所以距离约束对"坏 pose"结构性失明，加多少迭代、加多少 substep
都不会变。~~

**实测**：纯 XPBD 从**精确等距**的折叠中恢复了（`ch10032_tpose`，等距误差 2e-07，
excess ratio **0.29**，半衰期 **30 步**），而纯 GNN 一种损伤都不恢复（ratio 3.13–15.99）。
掰回翻折补丁也是纯 XPBD 最好（Δflip **+0.019** 对混合 +0.096 对纯 GNN +0.122）。

**错在哪**：

> **"单次投影表达不了某个位移" ≠ "该状态是整个闭环的不动点"。**

零空间是关于**一次线性化投影的像空间**的陈述。要从它推出动力系统的性质，还必须证明
**违反集为空**，而等距形变的违反集不空 —— 它是补丁的**边界环**（实测 106 条边界边对 1088 条
内部边，约 10%）。闭环里再加上重力、接触投影、228 sweep × 30 步的传播，足以把折叠解开。
**这一步被跳过了。**
...
| | ML 前置驱动 | XPBD 后置 |
| --- | --- | --- |
| **提供** | **正确的形状与动力学** —— C 干净 `edgeMAX` 1.291 / 1.533 对 B 的 3.667 / 3.382 | **鲁棒性** —— 抗高速（1.20×）、抗损伤（1.21×）、**最坏情况有界**、动画停后能松回 |
| **盲点** | **高速无界外推**（2× 播放 → `edgeMAX` 13.958，恶化 7.1×）、**放大损伤**（12.2×） | **形状错**（差 2.2–2.8×）；极端运动下峰值仍破（→ S9） |
| **逐资产人工成本** | 0（蒸馏一次） | 0（`target_length` 自动标定） |

两半的盲点确实互补，`C ≪ min(A, B)` 也确实有机制 —— 但机制是**"网络给形状、约束给鲁棒性"**，
不是"网络给流形回归、约束给几何界"。

痛点 3 的真实收益因此是**最坏情况有界**，而不是"网络把坏 pose 拉回流形"：
`ch10032_tpose` 上纯 GNN 被扰动后 `edgeMAX` 恶化 **12.2×**（1.345 → 16.371），混合只 1.37×，
且**混合受损后的最坏状态（1.768）优于纯 XPBD 未受损的最好状态（3.667）**。

这也让 §3.1 引用的那条消融结论（去掉整个图消息传递只差 14.7 分之一）不再构成威胁：前驱的职责
是**提供形状**，**这件事任何 ML 模型都能做，不必是 GNN**（→ S13）。

#### 两处如实记录

- **§8.3「基线必须包含 small steps」写进了方案，但从未执行。** 全仓库 `substep` 零命中，
  `project()` 在单个 Δt 下做 k 次迭代。G0 的 B 支是 `1 步 × 128 迭代` —— 恰好是 Macklin 2019
  *Small Steps* 指出的同预算最差用法。B 支还只跑过一个配置（`k128_teacher_sc0_bc1e-05_onesided`，
  从 C 的最优配置抄来），compliance / one-sided / sweep **一次都没为 B 调过**，且只拿到
  56% 的预算（1.21 ms 对 C 的 2.15 ms）。**所以"混合优于纯 XPBD"目前只对一个消融项成立。**
- **B 支的失败机制此前记错了。** `GATE_G0_RESULTS.md` / `PROGRESS.md` 写的是"垮掉、垂坠形状与
  teacher 不同"，而分解量说的是 `over` 占其分数的 **80–86%**（tpose 1.9287 / 2.2539，
  `under = 0`）：它是**边被拉长收不回来**，一个纯粹的收敛失败。这正是 substep 与非 Jacobi
  sweep 专门修的东西，所以那条"机制清楚"的记录必须撤回。
...
```
✅ G0  门禁：三方等成本对照（Python）                通过，见 §3.3 —— 但 B 支是消融项，见 §0.3
...
── 第三轮重排：先证痛点，再优化（见 §0.3 / §8.0）──
✅ S10  恢复力实验（痛点 3）        完成 2026-08-20。零空间预测被推翻；收益是最坏情况有界
✅ S10b 速度断裂点（痛点 2）        完成 2026-08-20。纯 GNN 在 2× 崩 7.1×，混合 1.29×
S10d 损伤幅度扫描 → 恢复盆         ← S10 最值得做的下一步：单一幅度分不出分布外
S10c 强 XPBD 对照：substep          ← §8.3 写了却从未执行
S11  渲染器穿模仪表                 ← 痛点 3 的渲染器确认；`hood_runtime.inl` 现无此列
S9   面积或二面角约束（痛点 2）      ← 已定位：sprint 2× 下混合峰值 9.076，退化率恶化 5.4×
S8   kernel occupancy 改造          ← 1.21 → 约 0.37 ms。纯性能，不检验任何论点，故降级
S12  第二件真实服装（痛点 1）        ← 零配置主张目前 N=1。**唯一有外部依赖的一项**
S13  MNN 作为可替换前驱             ← 战略最大；S10 证明前驱职责是"给形状"，与 GNN 无关
S7b  等预算训练（等 S8 定汇率）      选型目标改为带 XPBD 的分数
S5   碰撞：swept / 交错 / 自碰撞（独立预算）—— S11 的绝对穿模数字依赖它
S6   终态：学习型 V-Cycle
```

**排序理由**：S10 / S10b 已完成，它们确实推翻了一条论证并确立了正确的分工，证明这个顺序有用。
接下来 **S10d（恢复盆）优先于 S10c（substep）**：S10 已经让 B 拿到等预算并且**赢了**若干项，
所以再给 B 更多预算只会强化现有结论；而"多大的扰动还能恢复"是完全未知的。
S9 现在有了具体证据（sprint 2× 峰值 9.076），S8 仍是纯性能、让到 S9 之前即可。
...
> **⬆ 第三轮：这条曾被提为核心论证，随后当天被 S10 推翻，已回退为原状。**
> 零空间确实含所有等距形变，但"单次投影表达不了" **不等于**"该状态是闭环的不动点" ——
> 等距损伤的违反集不空（补丁边界环，实测约 10% 的边），闭环里的重力与接触足以解开它。
> 实测纯 XPBD 从精确等距的折叠中恢复（半衰期 30 步）。详见 §0.3 的撤回段与
> `results/RECOVERY_SPEED_RESULTS.md` §1。**所以本条只说明 warm start 无用，不说明别的。**
...
### 8.0 主指标：痛点导向，不引用 teacher（第三轮新增）

按 §0.3，下面三组才是验收指标。共同要求：**不得以 `cloth_rest` 或 teacher 轨迹作绝对基准** ——
前者由 §1.4 证明在真实服装上不可用，后者把命题偷换成"复现 HOOD"。

#### (a) 痛点 3：恢复力 —— `tools/recovery_probe.py`

参照物是**求解器自己的干净 rollout**：

```
forget(t) = RMS ‖ x_受损(t) − x_干净(t) ‖
```

两条 rollout 同权重、同场景，只差在第 `--warmup` 步被注入损伤。**恢复 = forget(t) 回落到
"未受损重跑"的水平**（`none` 控制组曲线）。控制组是必需的而非可选：它同时给出混沌分离的基线，
实测 A 支 90 步内自身就从 0.00038 涨到 0.00739。

注意 `forget` 增长**不等于**没恢复 —— 混沌系统里两条轨迹即使从同一状态出发也会分离。所以必须
同时报**不依赖轨迹同一性**的 pose 质量对照：受损运行末态的 `edge_p95` / `flipped` / 穿模数
对干净运行的同一量。
...
| 损伤 | 构造 | 对 Jᵀ 零空间的关系 |
| --- | --- | --- |
| `stretch` | 自由顶点绕质心缩放 1.30 | **完全可见** —— 正对照，>90% 的边被破坏 |
| `fold` | 沿网格边 BFS 取连通 patch，绕其最宽主轴 **180° 刚体旋转** | **精确等距**（实测 iso 误差 2e-7），只有约 10% 的边（边界环）报出违反 |
| `penetrate` | 同一 patch 沿最近代理法向**刚体平移**入体内 | 精确等距；但接触投影**能**看见 → 与 `fold` 构成判别对 |

**可falsify判据**：`fold` / `penetrate` 下 B 支不恢复而 A / C 恢复。**若 B 也恢复，痛点 3 上本
架构的论点即被推翻。**

> **❌ 该判据已于 2026-08-20 被违反：B 全部恢复，A 全部不恢复。** 见
> `results/RECOVERY_SPEED_RESULTS.md` §1 与 §0.3 的撤回段。修正后的读法是：
> **恢复力由 XPBD 提供，而 XPBD 单独会恢复到一个错的 pose**；混合的收益是**最坏情况有界**。
> 同时该文 §4 记录了本指标的两个失效模式（与 pose 质量矛盾、跨进程噪声约 2×），
> **可报结论应取自 pose 质量列**（`edgeMAX` / `edgeEnd`）。
>
> 下一步该做的是**损伤幅度扫描**（折叠 30°/90°/180°、穿入 0.04/0.08/0.12 m）——
> 恢复力真正该报的是**恢复盆的大小**，单一幅度分不出"扰动可恢复"与"分布外不可恢复"。

两条实施陷阱（都踩过，记下来）：

- **patch 必须沿网格边生长，不能用欧氏球。** 实测欧氏球会跨过腿抓到服装另一侧：n=200 时
  flatness 0.72 / 边界边 170 对内部 1040；BFS 测地 patch 是 0.19 / 106 对 1088。
- **fold 必须是刚体半周旋转，不能是反射。** 反射需要 patch 近平面才算折叠，而实测不平面，
  于是它破坏的边比 `stretch` 还多（RMS 0.189 对 0.025）—— 那测的就不是零空间了。
- `isometry_error` 必须在 float64 下用显式差值算。`torch.cdist` 展开成
  `‖a‖²+‖b‖²−2a·b`，float32 下米级坐标损失约 5e-4 m，比被测量本身还大。

#### (b) 痛点 2：速度断裂点 —— 同一工具的 `--frame-scales`

只有一个 `sprint` 只能给是非题。**抽帧即得精确时间缩放**（`train_student.py::frame_of`，
2× 就是隔帧取，无需插值），于是有了连续压力轴。

**`timestep` 刻意不缩放** —— 缩放 dt 会等比例给每个求解器更多预算，什么也测不出来。
`obstacle_frame = frame_of(step)` 配 `target_frame = frame_of(step+1)`，在 2× 下是两帧间隔，
这样 pin target 落在身体真正会到达的姿态上。

#### (c) 痛点 1：逐资产人工成本

一张两列表：本方案需要 authored 的量，对 ChaosCloth 需要 authored 的量。本方案侧目前是
**5 个全局量**（stretch/bend compliance、relaxation、contact_offset、iterations），
**逐资产为 0** —— `target_length` 由 `calibrate_from_trajectory` 自动测得。

⚠️ 这个主张目前 **N=1**：`ch10032_lower.vcloth2` 被 `ch10032_tpose` / `ch10032_sprint` /
`hml_001962` 三个场景共用，另一件 `hood_grid64.vcloth2` 是合成网格。**跨服装泛化完全无证据**
（**S12**）。

⚠️ 还有一处循环必须在 **S13** 前解决：`target_length` 现在从 **teacher** rollout 标定。换成
MNN 前驱后没有 teacher，只能从被纠正的驱动自己标定 —— 驱动拉长了就把拉长的长度烘成目标。
§0.2 已证标定可跨动作迁移，可行方向是只从慢速/静态动作标定（tpose 上 A 支 0.2410，驱动在那里
可信），或改用低分位数而非中位数。

#### 已知的仪表缺口（在补上之前不得给结论）

- **穿模没有绝对判据。** 最近代理半平面测不出"整个穿过肢体"的顶点：实测把 hem patch 推入
  0.12 m 报 28.5% 穿模，推入 0.25 m 只报 1%（已经从腿另一侧出去了）。**`_resolve_contacts`
  与 `hood_xpbd.comp` 用的正是同一个判据，所以求解器对隧穿同样失明。** 绝对数字要等 S5。
- **渲染器侧没有任何穿模列。** `hood_runtime.inl:1233-1311` 只有 `edge_length_ratio` /
  `triangle_area_ratio` / `degenerate` / `flipped`（**S11**）。

### 8.1 ⏬ 降级为回归检查：teacher-relative score

> **第三轮修订**：这一节原先是验收指标，现按 §0.3 降级。它衡量的是"能否廉价复现 HOOD"，
> 而项目要证的是 §8.0 的三条痛点，两者不是同一个命题。保留它的理由是它便宜且抓到过真问题
> （噪声底、模型 × XPBD 对照），所以继续作为**回归检查**跑，但不再单独决定任何去留。
...
### 8.3 ⚠️ 基线必须包含 small steps —— 写进了方案却从未执行

v1 这一条对，保留：基线要加入"更多子步、每子步一次 XPBD"，而不只是"一个大步里很多次迭代"。
在 dispatch 受限的现实下这一点尤其重要 —— 子步会成倍增加 dispatch，需要实测而非推理。

> **第三轮如实记录：这一条从未被执行。** 全仓库 `substep` 零命中，`project()` 是单个 Δt 下 k 次
> 迭代，G0 的 B 支就是 `1 步 × 128 迭代`。所以 §3.3 那句"C ≪ min(A, B)"里的 B **不是一个有
> 竞争力的纯 XPBD 求解器**，而是本混合架构的消融项，还只拿到 56% 的预算。
>
> 补做时的分工要写清楚：substep 只影响 `stretch` 类损伤的收敛，**对 §8.0(a) 的 `fold` /
> `penetrate` 按构造无效** —— 它改的是迭代调度，不是约束的零空间。所以 §8.0(a) 的中心判据
> 不必等 substep 就能测，而 §8.0(b) 的速度断裂点必须等它，否则 B 支的断点是被调度而非被物理
> 决定的。
...
| **从零空间推出动力系统性质** | 预先写死的判据被违反 | 补证"违反集为空"，否则只当作单次投影的陈述 | ❌ **已发生**：S10 实测纯 XPBD 从精确等距折叠中恢复（半衰期 30 步），见 §0.3 撤回段 |
| **用轨迹同一性衡量"恢复"** | 与 pose 质量列矛盾 | 结论取 pose 质量列；轨迹指标只用于定性判据 | ❌ **已发生**：C/hml 的 ratio 1.15 而 pose 正常；同配置跨进程 1.03 / 0.53 |
