...
| **S9** | **面积或二面角约束** | 已定位：`sprint_start` 第 60 步混合超冲 10.4 对纯 XPBD 的 2.5（渲染器 1% 内复现）；`ch10032_sprint` 2× 下 9.076；退化率恶化 5.4× |
| S10c | 强 XPBD 对照的 substep | `gnn-xpbd-v2.md` §8.3 写了却从未执行。B 已拿到等预算迭代数并赢了若干项，故优先级低于 S10d |
| S11 | 渲染器穿模仪表 | 逐支结构 JSON 已有（`hoodMeasureStructure`），但仍**无 penetration 列** |
