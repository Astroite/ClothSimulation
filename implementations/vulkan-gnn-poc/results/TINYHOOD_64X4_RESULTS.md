...
> **已被取代。** 这个 64×4 学生是第一次尝试，闭环会快速发散。后续分析显示它的宽/深权衡
> 是反的：GPU 成本约 ∝ `blocks × latent²`，所以深度便宜、宽度昂贵，而 64×4 恰好砍掉了
> 廉价的深度。替代它的 **32×12** 学生参数更少（200,227 对 286,275）、深度是 3 倍、
> CH10032 只需 `0.892 ms`，并且结构保持时长有数量级改善。
> 见 [`STUDENT_32X12_RESULTS.md`](STUDENT_32X12_RESULTS.md)。
> 本文保留作为失败记录与该架构的基线数据。
