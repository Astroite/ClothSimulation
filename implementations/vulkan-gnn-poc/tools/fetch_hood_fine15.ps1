...
$Checkpoint = Join-Path $WorkRoot 'hood_data/trained_models/fine15.pth'
$ExpectedArchiveSha256 = '3b68239bea3f298f9456680e34cf0204c90512ba1e43233febb375a90038a2a4'
$ExpectedCheckpointSha256 = 'bc92f1fb9a0ca1c9e476ad3981c3e4453bd66519ef16e6f2d6a52305c2aa13cb'
...
if (-not (Test-Path -LiteralPath $Checkpoint -PathType Leaf) -or (Get-FileHash -LiteralPath $Checkpoint -Algorithm SHA256).Hash.ToLowerInvariant() -ne $ExpectedCheckpointSha256) {
    ...
    & tar -xf $Archive -C $WorkRoot hood_data/trained_models/fine15.pth
    if ($LASTEXITCODE -ne 0) { throw 'Could not extract fine15.pth from the official HOOD archive' }
}

$CheckpointDigest = (Get-FileHash -LiteralPath $Checkpoint -Algorithm SHA256).Hash.ToLowerInvariant()
if ($CheckpointDigest -ne $ExpectedCheckpointSha256) { throw "Unexpected Fine15 SHA-256: $CheckpointDigest" }
...
& $Python (Join-Path $PocRoot 'tools/export_fine15.py') --checkpoint $Checkpoint
if ($LASTEXITCODE -ne 0) { throw 'Fine15 VHOOD export failed' }
if ($RemoveArchive -and (Test-Path -LiteralPath $Archive -PathType Leaf)) {
    Remove-Item -LiteralPath $Archive -Force
}
Write-Host "Fine15 VHOOD ready: $(Join-Path $WorkRoot 'hood_data/fine15.vhood')"
